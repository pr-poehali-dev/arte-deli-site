"""
Авторизация ARTE DELI: OTP по телефону, сессии.
POST /auth/send-otp — отправка кода (возвращает код в dev-режиме)
POST /auth/verify-otp — проверка кода, выдача токена
    ...
    try:
        ...
        if path.endswith("/send-otp") and method == "POST":
            ...
            cur.execute(f"INSERT INTO {SCHEMA}.otp_codes (phone, code, expires_at) VALUES (%s, %s, %s)",
                        (phone, code, expires))
            conn.commit()
            # In production — send SMS. For now return code in response (dev mode)
            return resp(200, {"success": True, "phone": phone, "dev_code": code,
                              "message": "Код отправлен (dev: смотри dev_code)"})
        ...
        if path.endswith("/verify-otp") and method == "POST":
            ...
            cur = conn.cursor()
            cur.execute(f"""
                SELECT id FROM {SCHEMA}.otp_codes
            ...
            if user_row:
                user_id, name, role = user_row
            else:
                cur.execute(f"INSERT INTO {SCHEMA}.users (phone, role) VALUES (%s, 'customer') RETURNING id, name, role",
                            (phone,))
        ...
        if path.endswith("/me") and method == "GET":
            ...
            # Get addresses
            cur = conn.cursor()
            cur.execute(f"SELECT id, address, apartment, entrance, floor, intercom, is_default FROM {SCHEMA}.user_addresses WHERE user_id=%s ORDER BY is_default DESC, id DESC", (user["id"],))
            addresses = [{"id": r[0], "address": r[1], "apartment": r[2], "entrance": r[3],
        ...
        if path.endswith("/profile") and method == "PUT":
            ...
            birth_date = body.get("birth_date", user["birth_date"])
            cur = conn.cursor()
            cur.execute(f"UPDATE {SCHEMA}.users SET name=%s, email=%s, birth_date=%s, updated_at=NOW() WHERE id=%s",
                        (name, email, birth_date or None, user["id"]))
            # Save address if provided
            if body.get("address"):
                cur.execute(f"UPDATE {SCHEMA}.user_addresses SET is_default=FALSE WHERE user_id=%s", (user["id"],))
                cur.execute(f"""INSERT INTO {SCHEMA}.user_addresses (user_id, address, apartment, entrance, floor, intercom, is_default)
                    VALUES (%s,%s,%s,%s,%s,%s,TRUE)""",
                    (user["id"], body["address"], body.get("apartment"), body.get("entrance"),
                     body.get("floor"), body.get("intercom")))
            conn.commit()
            return resp(200, {"success": True})
